...
The current App concept images are catalogued in [Design reference](design-reference/README.md).
They are broad references for design exploration, not the sole standard, final UI specifications,
or pixel-level acceptance criteria. Do not copy their layouts or UI styles; develop the interface
around actual user needs, accessibility, and device constraints. Domain definitions, product behavior,
and accepted ADRs govern any conflicts with the images. Sample content does not establish new requirements.
